"""
Lesson 17a: Run the sliding window across TRAIN subjects (never test),
using the CURRENT Stage 1 + Stage 2 pipeline, and collect every detection
that does NOT match a real lesion. These become new, realistic hard
negatives — actual whole-volume mistakes, not curated patch negatives.
"""
...
OUT_DIR = PATCHES_DIR / "wholevolume_hard_negatives"
OUT_DIR.mkdir(parents=True, exist_ok=True)

PATCH_SIZE = (16, 16, 8)
STRIDE = (8, 8, 4)
STAGE1_THRESHOLD = 0.5   # deliberately loose here — we WANT to catch borderline mistakes
MATCH_DISTANCE = 6
MAX_NEGATIVES_PER_SUBJECT = 40   # cap so one noisy subject doesn't dominate
...
def main():
    ...
    for subject_name in train_subjects:
        ...
        for i in range(0, len(candidates), 64):
            ...
            for center, prob in zip(batch, probs):
                if prob <= STAGE1_THRESHOLD:
                    continue
                # Is this actually near a real lesion? If yes, it's a correct detection, skip it.
                is_real = any(
        ...
        # Keep the most confident mistakes (the ones most worth correcting)
        ...
        for i, (center, prob) in enumerate(mistakes):
            ...
            np.save(subj_out / f"hardneg_{i}.npy", patch.astype(np.float32))

        total_saved += len(mistakes)
        print(f"{subject_name}: {len(mistakes)} whole-volume hard negatives mined")

    print(f"\nTotal new hard negatives: {total_saved}")
