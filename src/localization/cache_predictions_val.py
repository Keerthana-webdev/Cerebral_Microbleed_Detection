"""
Lesson 18: Run whole-volume inference on the VALIDATION set (never test)
using the current best model (v2), and cache raw probabilities. This
becomes our safe sandbox for further tuning/mining without touching test.
"""
...
CACHE_DIR = Path(__file__).resolve().parents[2] / "data" / "localization_cache_val"
CACHE_DIR.mkdir(parents=True, exist_ok=True)

PATCH_SIZE = (16, 16, 8)
STRIDE = (8, 8, 4)

# Current best model (v2) — this is our baseline for measuring further improvement
...
def process_subject(subject_name, batch_size=64):
    ...
    for i in range(0, len(candidate_centers), batch_size):
        ...
        with torch.no_grad():
            ...
            s2 = torch.sigmoid(stage2_model(patches_tensor)).squeeze(1).numpy()
...
def main():
    with open(SPLITS_DIR / "splits.json") as f:
        splits = json.load(f)
    val_subjects = splits["val"]   # CHANGED: val, not test

    print(f"Caching predictions for {len(val_subjects)} VALIDATION subjects (test set untouched)...\n")
    for subject_name in val_subjects:
        process_subject(subject_name)

    print(f"\nAll validation predictions cached to {CACHE_DIR}")
    print("✅ Test set was NOT touched in this step.")
