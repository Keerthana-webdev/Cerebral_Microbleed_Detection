"""
Lesson 21: Cache v3's whole-volume predictions on the VALIDATION set,
so we can compare v2 vs v3 without touching test.
"""
...
CACHE_DIR = Path(__file__).resolve().parents[2] / "data" / "localization_cache_v3_val"
CACHE_DIR.mkdir(parents=True, exist_ok=True)

PATCH_SIZE = (16, 16, 8)
STRIDE = (8, 8, 4)

stage1_model = DeeperCNN3D().to(DEVICE)   # CHANGED: deeper architecture
stage1_model.load_state_dict(torch.load(MODELS_DIR / "candidate_detector_v3_best.pt", map_location=DEVICE))
stage1_model.eval()

stage2_model = SimpleCNN3D().to(DEVICE)   # mimic classifier unchanged
...
def main():
    ...
    print(f"\nAll v3 validation predictions cached to {CACHE_DIR}")
